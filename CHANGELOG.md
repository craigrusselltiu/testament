...
### Fixed
- **Stale test names after renaming** - Discovery cache only checked `.csproj` and `.dll` modification times, so renaming a test method in a `.cs` file could still show the old name. Cache invalidation now also tracks the newest `.cs` source file modification time.
